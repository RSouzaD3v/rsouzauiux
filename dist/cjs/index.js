...
const StyledButton = dt.button `
    ...
    ${({ hoverEvent, bgcolor }) => hoverEvent &&
    `&:hover {
          background-color: ${bgcolor ? curriedDarken$1(0.1, bgcolor) : curriedDarken$1(0.1, '#007bff')};
      }`}
`;

const Button = ({ text, onClick, styled }) => {
    return (jsxRuntimeExports.jsx(StyledButton, { onClick: onClick, bgcolor: styled?.bgcolor, color: styled?.color, borderRadius: styled?.borderRadius, padding: styled?.padding, hoverEvent: styled?.hoverEvent, children: text }));
};
